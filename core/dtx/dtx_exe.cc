...
bool DTX::TxExe(coro_yield_t &yield , bool fail_abort){
  ...
  for (size_t i=0; i<read_only_set.size(); i++) {
    ...
    if (!item.is_fetched) { 
      ...
      // assert(bl_rid.page_no_ == real_rid.page_no_ && bl_rid.slot_no_ == real_rid.slot_no_);
      if(rid.page_no_ == -1) {
        // Data not found
...
bool DTX::TxExe_TS(coro_yield_t& yield, bool fail_abort) {
  // MV2PL+No-wait
  //  // LOG(INFO) << "TxExe" ;
  int sleep_time = 0;
  struct timespec start_time, end_time;
    ...
    futures.emplace_back(thread_pool->enqueue([&, idx, rid, task](){
      ...
      if(SYSTEM_MODE == 0 || SYSTEM_MODE == 1 || SYSTEM_MODE == 3){
        ...
        *item.item_ptr = *data_item;
        ...
        table_id_t now_table_id = item.item_ptr->table_id;
        auto data = FetchSPage(yield , now_table_id , rid.page_no_);
        *item.item_ptr = *GetDataItemFromPageRO(now_table_id , data , rid);
        item.is_fetched = true;
        ReleaseSPage(yield , item.item_ptr->table_id , rid.page_no_);
      } else{
        assert(false);
