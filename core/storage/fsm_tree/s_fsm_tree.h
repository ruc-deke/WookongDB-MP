...
// 常量定义
constexpr uint32_t S_FSM_META_PAGE_ID = 1;
constexpr uint32_t S_FSM_ROOT_PAGE_ID = 2;

// 每个FSM页面管理的最大叶子节点数
constexpr uint32_t S_LEAVES_PER_PAGE = 1024;
// 每个内部FSM页面管理的最大子FSM页面数
constexpr uint32_t S_CHILDREN_PER_PAGE = 512;

// FSM页面类型
enum class S_FSMPageType : uint8_t {
    META_PAGE = 0,      // 元数据页面
...
// 空间分类
enum class S_SpaceCategory : uint8_t {
    FULL = 0,           // 0-10% 空闲
...
struct S_FSMMetaData {
    ...
    table_id_t table_id;            // 关联的表ID(smallbank 0-1)
    uint8_t  reserved[64];          // 保留区域
};

// FSM页面头
struct S_FSMPageHeader {
    uint32_t magic_number;          // 标识
    uint32_t page_id;               // 页面ID
    S_FSMPageType page_type;          // 页面类型
...
class S_FSMNode {
public:
    S_FSMNode() : value(static_cast<uint8_t>(S_SpaceCategory::FULL)), is_dirty(false) {}
    explicit S_FSMNode(S_SpaceCategory cat) : value(static_cast<uint8_t>(cat)), is_dirty(false) {}
    ...
    void set_value(S_SpaceCategory cat) { set_value(static_cast<uint8_t>(cat)); }
...
public:
    ...
    std::vector<S_FSMNode> nodes;
    std::vector<uint32_t> child_page_ids;
    bool is_loaded;
    bool is_dirty;
    
    S_FSMPageData() : is_loaded(false), is_dirty(false) {
    ...
    void initialize_leaf_page(uint32_t page_id, uint32_t parent_id, uint32_t level,
        ...
        header.page_type = S_FSMPageType::LEAF_PAGE;
        ...
        nodes.resize(header.node_count, S_FSMNode(S_SpaceCategory::FULL));
        is_loaded = true;
        is_dirty = true;
    }
    ...
    void initialize_internal_page(uint32_t page_id, uint32_t parent_id, uint32_t level,
        ...
        header.page_id = page_id;
        header.page_type = S_FSMPageType::INTERNAL_PAGE;
        header.parent_page = parent_id;
        header.level = level;
...
class S_SecFSM {
public:
    explicit S_SecFSM(DiskManager *dm, StorageBufferPoolManager *buffer_pool,table_id_t table_id , const std::string bench_name);
    ~S_SecFSM();
    
    // 初始化FSM树
    bool initialize(table_id_t table_id, uint32_t initial_heap_pages);
...
private:
    ...
    bool load_fsm_page(uint32_t fsm_page_id);
    bool flush_fsm_page(uint32_t fsm_page_id);
    S_FSMPageData* get_page(uint32_t page_id);
    ...
    uint32_t search_in_leaf_page(S_FSMPageData& leaf_page, uint8_t required_category);
    ...
    bool serialize_page(S_FSMPageData& page_data, char* buffer, uint32_t size);
    bool deserialize_page(S_FSMPageData& page_data, const char* buffer, uint32_t size);
    ...
    bool is_leaf_node(const S_FSMPageData& page_data, uint32_t index) const;
    void ensure_child_vector(S_FSMPageData& page_data);
...
private:
    ...
    S_FSMMetaData meta_;
    std::unordered_map<uint32_t, S_FSMPageData> fsm_pages_;
...
// 简易断言辅助
static void S_Assert(bool cond, const char* msg) {
    if (!cond) {
