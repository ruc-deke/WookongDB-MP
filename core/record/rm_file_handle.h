...
class RmFileHandle {      
   ...
   public:
    RmFileHandle(DiskManager *disk_manager, StorageBufferPoolManager *buffer_pool_manager, int fd)
    ...
    std::unique_ptr<RmRecord> get_record(const Rid &rid, TxnLog* txn) const;

    Rid insert_record(itemkey_t key, char *buf, TxnLog* txn);

    Rid insert_nextpage_record(itemkey_t key, char *buf, TxnLog* txn);

    void insert_record(const Rid &rid, char *buf);

    void delete_record(const Rid &rid, TxnLog* txn);

    void update_record(const Rid &rid, char *buf, TxnLog* txn);

    RmPageHandle create_new_page_handle(TxnLog* txn);

    RmPageHandle fetch_page_handle(page_id_t page_no) const;

    DiskManager *disk_manager_;
    StorageBufferPoolManager *buffer_pool_manager_;
    int fd_;        // 打开文件后产生的文件句柄
    RmFileHdr file_hdr_;    // 文件头，维护当前表文件的元数据

   private:
    RmPageHandle create_page_handle(TxnLog* txn);
