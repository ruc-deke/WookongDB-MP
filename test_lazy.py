...
compute_server_ports = [22,22,22]
compute_server_usernames = ['root','root','root','root']
compute_server_passwords = ['wljwlj123','wljwlj123','wljwlj123']
...
# 配置热点账户的数量
def set_workload_accounts(bench , hot_num):
    if bench == 'smallbank':
        cfg_path = os.path.join(workspace, 'config', 'smallbank_config.json')
        with open(cfg_path, 'r') as f:
            data = json.load(f)
        data['smallbank']['num_hot_accounts'] = int(hot_num)
        data['smallbank']['attempted_num'] = int(attempt_num)
...
def start_remote_services_checked(client, primary_build_dir, fallback_build_dir=None):
    ...
    cmd_storage = f"cd {primary_build_dir}/storage_server && ./storage_pool"
    cmd_remote = f"cd {primary_build_dir}/remote_server && ./remote_node"
            ...
            for txh in tx_hot_list:
                for cr in cross_ratios:
                    for mode in modes:
                        ...
                        ok = start_remote_services_checked(rs_client, remote_build_dir, fallback_build_dir=os.path.join("/usr/local/exper/Hybrid_Cloud_MP", "build"))
